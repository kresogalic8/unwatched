#!/usr/bin/env bash
# Deploy an already-published, checked-out release. Never reads local .env files.
set -euo pipefail
cd "$(dirname "$0")/.."
: "${DO_APP_ID:?}" "${DO_REGISTRY:?}" "${UW_PUBLIC_URL:?}" "${RELEASE_TAG:?}"
: "${NEXT_PUBLIC_SUPABASE_URL:?}" "${NEXT_PUBLIC_SUPABASE_ANON_KEY:?}"
[[ "$RELEASE_TAG" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]
export RELEASE_VERSION="${RELEASE_TAG#v}" COMMIT_SHA="$(git rev-parse HEAD)"
[[ "$(node -p 'require("./package.json").version')" == "$RELEASE_VERSION" ]]
[[ "$(git rev-parse "$RELEASE_TAG^{commit}")" == "$COMMIT_SHA" ]]
export IMAGE_TAG="$RELEASE_TAG-$COMMIT_SHA"
