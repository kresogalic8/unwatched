...
export function Page({children,signedIn=true}:{children:React.ReactNode;signedIn?:boolean}) {
 ...
 return <DigestLayout active="account" signedIn={signedIn}><div className={s.page}><header className={s.hero}><Label>Your account</Label><h1>{title}</h1><p>{path==="/account/brain"?"Choose what powers your citizen’s decisions.":path==="/account/credits"?"Your allowance, your balance, your choice.":"Your citizens, their settings, and how you stay in touch."}</p></header><nav className={s.nav} aria-label="Account settings">{[["/account","Citizens & settings"],["/account/credits","Credits & plan"],["/account/brain","Who thinks"]].map(([href,label])=><Link key={href} href={href!} aria-current={path===href?"page":undefined}>{label}</Link>)}</nav><div className={s.body}>{children}</div></div></DigestLayout>;
}
