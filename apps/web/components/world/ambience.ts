...
export const ONESHOTS = ["gull", "bell", "horn", "creak", "thunder", "bark", "meow", "cluck", "flap", "oars", "step", "boom"] as const;
...
export class Ambience {
  ...
  /** a firework: the thump as it opens, far off, then the crackle of the stars falling */
  private boom(dest: AudioNode): void {
    if (this.shot("boom", 0.5, dest)) return; const c = this.ctx!, t = c.currentTime;
    const src = c.createBufferSource(); src.buffer = noiseBuffer(c, 1.2); const f = c.createBiquadFilter(); f.type = "lowpass"; f.frequency.setValueAtTime(420, t); f.frequency.exponentialRampToValueAtTime(60, t + 0.9);
    const g = c.createGain(); g.gain.setValueAtTime(0, t); g.gain.linearRampToValueAtTime(0.9, t + 0.02); g.gain.exponentialRampToValueAtTime(0.001, t + 1.1); src.connect(f); f.connect(g); g.connect(dest); src.start(t); src.stop(t + 1.2);
    for (let i = 0; i < 14; i++) { const at = t + 0.25 + Math.random() * 0.9; const k = c.createBufferSource(); k.buffer = noiseBuffer(c, 0.03); const kf = c.createBiquadFilter(); kf.type = "highpass"; kf.frequency.value = 2500 + Math.random() * 3000; const kg = c.createGain(); kg.gain.value = 0.08 + Math.random() * 0.1; k.connect(kf); kf.connect(kg); kg.connect(dest); k.start(at); }
  }
