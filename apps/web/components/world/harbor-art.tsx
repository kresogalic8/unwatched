...
export function House({u,v,w,d,h,name,color="#ece3cc",side="#c4c5ac",roof=0,lit=false,cafe=false}:{u:number;v:number;w:number;d:number;h:number;name:string;color?:string;side?:string;roof?:number;lit?:boolean;cafe?:boolean}){
 ...
 return <Face key={`${i}-${j}`} points={[q(a,va,h+rise*(1-j/5)),q(b,va,h+rise*(1-j/5)),q(b,vb,h+rise*(1-(j+1)/5)),q(a,vb,h+rise*(1-(j+1)/5))]} fill={tileColors[(i+j*3+roof)%tileColors.length]!} stroke="#8f6048" width={.38}/>;
 }))}
...
export function Tree({u,v,scale=1}:{u:number;v:number;scale?:number}){
 ...
 {Array.from({length:42},(_,i)=>{const a=i*2.399,r=8+Math.sqrt(i/42)*40;const xx=cos(a)*r, yy=-73+sin(a)*r*.6;return <ellipse key={i} cx={xx} cy={yy} rx={10+noise(i)*7} ry={8+noise(i+2)*5} fill={["#738766","#829775","#99a67c","#a8af85","#667e61"][i%5]}/>;})}
...
export function Table({u,v,parasol=false}:{u:number;v:number;parasol?:boolean}){const [x,y]=P(u,v);return <g transform={`translate(${x} ${y})`}>
 <ellipse cy="4" rx="35" ry="11" fill="#607357" opacity=".14"/><path d="M0 -22V1M-8 2L0 -2L10 1" stroke="#6b785c" strokeWidth="3"/><ellipse cy="-23" rx="23" ry="10" fill="#d6c49b" stroke="#9c9875" strokeWidth="1"/>
 ...
 {[-1,1].map(n=><g key={n} transform={`translate(${n*32} 0)`}><path d="M-8 -10V6M8 -10V6M-8 -9H8M-8 -9V-23H8V-9" fill="none" stroke="#758166" strokeWidth="2.2"/><path d="M-8 -16H8" stroke="#758166" strokeWidth="1.4"/></g>)}
