...
export function World({ frameTap, photoControl, rewindControl: rewindHandle, onRewind, mineId, onSelect, view, effects = true, observer = false, onSnapshot, focusId, onViewChange, apiUrl = API, selectedId = null, spotlight = null, miniature: miniatureProp, onMiniatureChange, compact = false }: { /** the miniature look, when the page holds it (the town page's view switcher); left out, the world keeps its own */ miniature?: boolean; onMiniatureChange?: (on: boolean) => void; /** the town page's own chrome: no minimap, and only zoom and sound at the island's edge */ compact?: boolean; /** called with the canvas right after every frame is drawn, while it still holds the picture (for filming) */ frameTap?: { current: ((canvas: HTMLCanvasElement) => void) | null }; /** filled with a way to take the island's picture as it is on screen */ photoControl?: { current: (() => Photo | null) | null }; /** filled with the controls of the day played back, for the page's scrubber */ rewindControl?: { current: RewindControl | null }; onRewind?: (state: RewindState) => void; apiUrl?: string; mineId: string | null; onSelect: (a: PublicAgent | null) => void; view: "street" | "map" | "cinema"; effects?: boolean; observer?: boolean; focusId?: string | null; onViewChange?: (view: "street" | "map" | "cinema") => void; onSnapshot?: (snapshot: WorldSnapshot) => void; selectedId?: string | null; spotlight?: { id: number; actors: string[]; place: string | null; at: number } | null }) {
