...
describe('plan allowance changes',()=>{
 it('grants purchased quotas immediately without replenishing them on repeated application or restart',async()=>{
  ...
  await billing.setPlan('owner','resident');billing.applyPlan(a);expect(a.budget.tier1Left).toBe(50);expect(a.budget.tier2Left).toBe(6);expect(a.budget.reflectionIncluded).toBe(true);
 ...
 it('visitor does not receive an included nightly reflection',async()=>{
  ...
  await billing.setPlan('owner','visitor');billing.applyPlan(a);expect(a.budget.reflectionIncluded).toBe(false);expect(a.budget.planningIncluded).toBe(true);
 });
});
